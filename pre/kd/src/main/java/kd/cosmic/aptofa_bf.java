...
/**
 * 描述: 财务应付单下固定资产报废单，给下游字段赋值，插件位置：对应的转换规则
 * 开发者: 易佳伟
 * 创建日期: 1期
 * 关键客户：顾问夏晓君
 * 已部署正式：ture
...
public class aptofa_bf extends AbstractConvertPlugIn {
    ...
    public void afterConvert(AfterConvertEventArgs e) {
        ...
        for (DynamicObject changeBill : changeBillObjs) {
            ...
            Date ezobsqrq = setDateDifPeriod(changeBill.getDynamicObject("org"), Boolean.FALSE.booleanValue());
            Set<Long> curRealCard = FaUtils.getCurRealCard(ezobsqrq, Long.valueOf(changeBill.getLong("org.id")), Boolean.valueOf(false), realSet);
            ...
            if (engnnerRealMap.containsKey(Long.valueOf(aPId)))
                for (Long real : realcards) {
                    ...
                    for (DynamicObject fincard : fincs) {
                        Date ezobsqrq = setDateDifPeriod(changeBill.getDynamicObject("org"), Boolean.FALSE.booleanValue());
                        changeBill.set("changedate", ezobsqrq);
...
//              throw new KDBizException(ResManager.loadKDString(", "EngineeringToFinCardGenerate_0", "fi-fa-business", new Object[0]));
                        ...
                        //BigDecimal originalvalAfter = fincard.getBigDecimal("originalval").add(ApInfo.getBigDecimal("e_pricetaxtotal").multiply(exchangeRate));
                        //originalvalDy.set("aftervalue", originalvalAfter.setScale(amtprecision, RoundingMode.HALF_UP));
                        originalvalDy.set("aftervalue", fincard.get("originalval"));
                        originalvalDy.set("realcard1", fincard.get("realcard"));
                        originalvalDy.set("fincard1", fincard);
                        originalvalDy.set("reason", "部分报废申请");
                        ...
                        Date sqrq = doAp.getDate("ezob_sqrq");
                        originalvalDy.set("bizdate1", sqrq);
                        preresidualvalDy.set("field", "fa_card_fin.preresidualval");
                        preresidualvalDy.set("beforevalue", fincard.get("preresidualval"));
                        //BigDecimal preresidualvalAfter = fincard.getBigDecimal("preresidualval").add(ApInfo.getBigDecimal("e_pricetaxtotal").multiply(netResidualValRate).divide(new BigDecimal(100), 4).multiply(exchangeRate));
                        //preresidualvalDy.set("aftervalue", preresidualvalAfter.setScale(amtprecision, RoundingMode.HALF_UP));
                        preresidualvalDy.set("aftervalue", fincard.get("preresidualval"));
                        preresidualvalDy.set("realcard1", fincard.get("realcard"));
                        preresidualvalDy.set("fincard1", fincard);
                        preresidualvalDy.set("reason", "部分报废申请");
                        ...
                        preresidualvalDy.set("bizdate1", sqrq);
    ...
    private BigDecimal getExchangeRate(DynamicObject fincard, DynamicObject doAp, Map<String, BigDecimal> exchangeRateMapCache) {
        ...
        long currencyId = assetBook.getLong("basecurrency.id");//原本取的是源单的币别，这里改为取固定资产账簿的默认币别
    ...
    private static Map<String, DynamicObject> getApMap(Set<Long> sourceids, Set<Object> realCardIdSet, Set<Object> newRealCard, Map<Long, DynamicObject> enginnerBillMap) {
        ...
        //加载财务应付单元数据
        DynamicObject[] aPs = BusinessDataServiceHelper.load("ezob_bfbfsqd", "entryentity,entryentity.creator,entryentity.ezob_gdzcbm,entryentity.ezob_gdzcbm.bizstatus,entryentity.ezob_gdzcbm.masterid,entryentity.ezob_gdzcbm.number,ezob_createtime,basecurrency,ezob_sqrq", new QFilter[] { qSourseid });
